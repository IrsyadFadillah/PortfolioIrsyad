...
const ADMIN_PASSWORD = 'irsyad2024'; // Change this
...
function getProjects() {
  ...
  } catch { return getDefaultProjects(); }
}
...
function getDefaultProjects() {
  // Return empty array; in production this would fetch from data/projects.json
  return [
    ...
    { id: 3, title: "Customer Churn Prediction", description: "Python ML model to predict customer churn with 87% accuracy.", tools: ["Python", "SQL", "Power BI"], image: "", github: "https://github.com/irsyadfadillah", featured: true, category: "Data Analytics" }
  ];
}
...
document.getElementById('add-project-form')?.addEventListener('submit', (e) => {
  ...
  const toolsRaw = form.querySelector('#proj-tools').value.trim();
  const image = form.querySelector('#proj-image').value.trim();
  const github = form.querySelector('#proj-github').value.trim();
  const featured = form.querySelector('#proj-featured').checked;
  const category = form.querySelector('#proj-category').value;

  if (!title || !description) {
    showAlert('alert-error', '✗ Title and description are required.');
    return;
  }
  ...
  const newProject = {
    id: Date.now(),
    title, description, tools, image, github, featured, category
  };

  projects.push(newProject);
  saveProjects(projects);

  showAlert('alert-success', `✓ Project "${title}" added successfully!`);
  form.reset();
  loadProjectList();
  renderJsonOutput(projects);
});
...
function loadProjectList() {
  ...
  if (projects.length === 0) {
    container.innerHTML = '<p style="color:var(--text3);font-family:DM Mono,monospace;font-size:.78rem;text-align:center;padding:24px">No projects yet. Add one above.</p>';
    return;
  }
  ...
  container.innerHTML = projects.map(p => `
    <div class="project-item" data-id="${p.id}">
      <div>
        ...
        <div class="project-item-meta">${p.category} · ${(p.tools || []).join(', ')}</div>
      </div>
